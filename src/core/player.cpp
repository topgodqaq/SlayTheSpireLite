...
Player::Player(int maxHP, int maxEnergy)
    : currentHp(maxHP), maxHp(maxHP), currentEnergy(maxEnergy), maxEnergy(maxEnergy)
{
    // 初始化卡组
    for (int i = 0; i < 5; i++)
    {
        deck.append(Card("strike", "打击", Card::Attack, 1, 6, "", Card::Common, false, "images/cards/strike.png"));
    }
    for (int i = 0; i < 4; i++)
    {
        deck.append(Card("defend", "防御", Card::Skill, 1, 5, "", Card::Common, false, "images/cards/defend.png"));
    }
    // 痛击卡
    deck.append(Card("bash", "痛击", Card::Attack, 2, 8, "", Card::Uncommon, false, "images/cards/bash.png"));
}
...
QString Player::getStatusText() const
{
    ...
    if (weak > 0)
        statuses << QString("虚弱%1").arg(weak);
    if (vulnerable > 0)
        statuses << QString("易伤%1").arg(vulnerable);
    if (frail > 0)
        statuses << QString("脆弱%1").arg(frail);
